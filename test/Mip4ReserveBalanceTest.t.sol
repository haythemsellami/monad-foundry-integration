...
contract MonadNineMip4ReserveBalanceTest is Test {
    ...
    function test_staticcallRejected() public view {
        (bool success, bytes memory ret) = RESERVE_BALANCE.staticcall(
            abi.encodeWithSelector(DIPPED_INTO_RESERVE_SELECTOR)
        );
    ...
    function test_delegatecallRejected() public {
        (bool success, bytes memory ret) = RESERVE_BALANCE.delegatecall(
            abi.encodeWithSelector(DIPPED_INTO_RESERVE_SELECTOR)
        );
    ...
    function test_callcodeRejected() public {
        ...
        bool success =
            caller.callViaCallcode(abi.encodeWithSelector(DIPPED_INTO_RESERVE_SELECTOR));
    ...
    function test_staticContextPropagationRejected() public {
        ...
        (bool outerSuccess, bytes memory ret) = address(caller).staticcall(
            abi.encodeCall(
                ReserveBalanceCaller.forwardCall,
                (abi.encodeWithSelector(DIPPED_INTO_RESERVE_SELECTOR))
            )
        );
    ...
    function test_nonzeroValueRejected() public {
        ...
        (bool success, bytes memory ret) = RESERVE_BALANCE.call{value: 1}(
            abi.encodeWithSelector(DIPPED_INTO_RESERVE_SELECTOR)
        );
    ...
    function test_extraInputRejected() public {
        (bool success, bytes memory ret) = RESERVE_BALANCE.call(
            abi.encodePacked(DIPPED_INTO_RESERVE_SELECTOR, bytes1(0x01))
        );
...
contract ReserveBalanceCaller {
    ...
    function forwardCall(bytes calldata data)
        external
    ...
    function callViaCallcode(bytes calldata data) external returns (bool success) {
        ...
        assembly {
            success := callcode(
                gas(), target, 0, add(callData, 0x20), mload(callData), 0, 0
            )
        }
    }

    function callWithData(bytes calldata data)
        external
        payable
        returns (bool success, bytes memory ret)
    {
...
contract DelegatedReserveProbe {
    IReserveBalance constant RESERVE =
        IReserveBalance(address(0x0000000000000000000000000000000000001001));
    ...
    function childSpendRevertAndRecord(RefundSink sink, uint256 amount) external {
        (bool ok, bytes memory ret) = address(this).call(
            abi.encodeCall(DelegatedReserveProbe.childSpendRevert, (sink, amount))
        );
